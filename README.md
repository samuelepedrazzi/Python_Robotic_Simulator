...
Using `pip`, it should be installed the pygame dependence by the command: `pip install hg+https://bitbucket.org/pygame/pygame`, otherwise it can be used the operating system's package manager. `PyYAML` and `PyPyBox2D` can be clearly obtained by `pip install PyYAML` and `pip install PyPyBox2D` or it's just fine from `pip` or `easy_install`.
